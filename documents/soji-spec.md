...
title: soji — 정적 사이트 생성기 명세
date: 2026-06-04
tags: [soji, infra, spec]
draft: false
---
...
도구 명령은 `soji`, 단일 콘텐츠 개념은 `item`이라 부른다.
...
`build.py`/`deploy.sh`를 서브커맨드를 가진 도구 `soji`로 일반화한다.

- `soji build` — 7장 파이프라인 실행, `output_dir` 생성.
- `soji serve` — 빌드(드래프트 포함) 후 `output_dir`를 정적 서빙. **v1은 watch·라이브 리로드 없음**(후속, §12).
- `soji deploy` — `build` 후 설정의 배포 전략 실행(10장).
- `soji new` — 새 converter 아이템 스캐폴드. **`item` 스키마(1.3)의 required 필드 + default로 frontmatter 골격을 생성**한다.
